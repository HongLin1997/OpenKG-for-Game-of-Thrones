...
for index, lin in enumerate(lines):
    ...
    for i, (tk, pos) in enumerate(zip(temp['hanlp_tokens'],
        ...
        if ((pos.startswith('nr') and len(tk)>1) or \
            ...
            nrs_in_sentence.add((tk, pos, postition))
            all_nrs.add((tk, pos))
    ...
    for ent in entity_pairs:
        ((e1, pos, postition), (e2, pos, postition)) = ent
        relation = None
        if true_triples.get((e1,e2),None)!=None:
            for r in true_triples.get((e1,e2)):
                ...
                if temp not in corpus:
                    corpus += [temp] 
                    postiive_count[relation] = postiive_count.get(relation,0)+1
                    
        elif true_triples.get((e2,e1),None)!=None:
        ...
        elif potential_triples.get((e1,e2),None)!=None:
        ...
        elif potential_triples.get((e2,e1),None)!=None:
